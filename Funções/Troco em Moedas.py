...
10, 5 e 1 centavos que deve dar, de forma a minimizar a quantidade de moedas do
troco.

A Entrada consiste de:

    A função troco deve ser chamada para qualquer valor arbitrário de troco
    definidos nos casos de teste, que é um número inteiro x, o valor do troco em
    centavos.
...
# Basta apenas mostrar a quantidade de moedas restantes, dado que este número sempre será 
# o mesmo que ele mesmo dividido por 1.

def troco(n:int):
    print(f"{n//50} moedas de 50 centavos")
    n -= 50*(n//50)
    print(f"{n//25} moedas de 25 centavos")
    n -= 25*(n//25)
    print(f"{n//10} moedas de 10 centavos")
    n -= 10*(n//10)
    print(f"{n//5} moedas de cinco centavos")
    n -= 5*(n//5)
    print(f"{n} moedas de 1 centavo")
