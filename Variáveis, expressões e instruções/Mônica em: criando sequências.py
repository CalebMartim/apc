...
A fórmula de Mônica para criar uma sequência consiste em alguns elementos: três
números inteiros a , b e c, todos maiores que zero e duas sequências p1 e p2
...
Mônica gostaria de automatizar esse processo elaborando um programa com toda a
lógica que ela desenvolveu, mas como o PC dela foi pro conserto, ela não pode
fazer. Você poderia fazer esse favor a ela? 

A Entrada consiste de:

    Três números inteiros positivos. Duas strings, uma em cada linha, como
...
# Recebe os 5 valores, três numeros e duas sequências de strings
a = int(input())
b = int(input())
c = int(input())
p1 = input()
...
# Mostra cada string no modo pedido
print((a+c)*(p1*(a+b) + p2*(b+c)))
